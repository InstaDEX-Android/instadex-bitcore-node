...
The InstaDEX Service is a Node.js interface to [InstaDEX Core](https://github.com/instadex-android/instadex-core) for querying information about the instadex block chain. It will manage starting and stopping `instadexd` or connect to several running `instadexd` processes. It uses an instance of a InstaDEX with additional indexes enabled for querying information about addresses and blocks. Results are cached for performance and there are several additional API methods added for common queries.

## Configuration

The default configuration will include a "spawn" configuration in "instadexd". This defines the location of the block chain database and the location of the `instadexd` daemon executable. The below configuration points to a local clone of `instadex`, and will start `instadexd` automatically with your Node.js application.
...
It's also possible to connect to separately managed `instadexd` processes with round-robin quering, for example:
