...
var bitcore = require('instadex-bitcore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var BitcoinRPC = require('instadex-bitcoind-rpc');
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getBlockchainInfo', this, this.getBlockchainInfo, 0],
    ['getNetworkInfo', this, this.getNetworkInfo, 0],
    ['getWalletInfo', this, this.getWalletInfo, 0],
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './instadex.conf');

  log.info('Using instadex config file:', configPath);
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('InstaDEX Daemon Ready');
          callback();
        });
      });
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet/instadex.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/instadex.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('InstaDEX Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('InstaDEX Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/instadexd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned instadex process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean instadex process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting instadex process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('InstaDEX process unexpectedly exited with code:', code);
        log.warn('Restarting instadex child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('InstaDEX process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn instadexd.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to instadexd.'));
    }
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('InstaDEX configuration options "spawn" or "connect" are expected'));
    }
...
Bitcoin.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  function addInputsToTx(tx, result, counts) {
    tx.inputs = [];
    tx.inputSatoshis = 0;
    for(var inputIndex = 0; inputIndex < result.vin.length; inputIndex++) {
      ...
      tx.inputs.push({
        ...
        num_inputs: input.num_inputs || null,
        ...
        witnessStack: input.txinwitness
      });
    }
  }

  function addOutputsToTx(tx, result, counts) {
    tx.outputs = [];
    tx.outputSatoshis = 0;
    for(var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
      ...
      if (out.type == 'blind')
      {
        ...
        tx.outputs.push({
          ...
          rp_exponent: out.rp_exponent,
          rp_mantissa: out.rp_mantissa,
          rp_min_value: out.rp_min_value,
          rp_max_value: out.rp_max_value,
          rp_size: out.rangeproof.length / 2
        });
      } else
      if (out.type == 'anon')
      {
        var pk = new bitcore.PublicKey(out.pubkey);
        tx.outputs.push({
          type: out.type,
          satoshis: 0,
          //pubkey: out.pubkey,
          address: bitcore.Address.fromPublicKey(pk, self.node.network).toString(),
          valueCommitment: out.valueCommitment,
          data: out.data_hex,
          rangeproof: out.rangeproof,
          rp_exponent: out.rp_exponent,
      ...
      {
        process.stdout.write("Unknown output type: " + obj.type + "\n");
      }
    }
  }
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        var tx = {
          ...
          height: (result.height || result.confirmations) ? result.height : -1,
        ...
        counts.nct = 0;
        counts.nrct = 0;
        addInputsToTx(tx, result, counts);
        ...
        if (!tx.coinbase) {
          if (tx.feeSatoshis == -1)
          {
            if (counts.nct == 0 && counts.nrct == 0)
            {
              tx.feeSatoshis = tx.inputSatoshis - tx.outputSatoshis;
            } else
            {
              // fee must be encoded in data output
...
Bitcoin.prototype.getBlockchainInfo = function(callback) {
  var self = this;
  this.client.getBlockchainInfo(function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
...
Bitcoin.prototype.getNetworkInfo = function(callback) {
  var self = this;
  this.client.getNetworkInfo(function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
